...
import {
  ...
  Lock, Eye, EyeOff, KeyRound, CheckCircle2, AlertCircle, RefreshCw, Plus,
} from 'lucide-react';
...
export default function AccountPage() {
  const {
    ...
    resendVerificationEmail, sendWhatsAppOtp, verifyWhatsAppOtp,
  } = useAuth();
  ...
  // Editing phone (for verified users changing number)
  ...
  async function savePhoneEdit() {
    const ok = await updateProfile({ phone: normalisePhone(phoneVal) });
    if (ok) { setEditingPhone(false); setPhoneVerifying(false); }
  }
  ...
  async function handleAddEmail() {
    if (!newEmail.trim().includes('@')) { toast.error('Enter a valid email'); return; }
    setEmailBusy(true);
    const ok = await addEmailToAccount(newEmail.trim().toLowerCase());
    setEmailBusy(false);
    if (ok) {
      toast.info('Verification email sent! Click the link to verify.', { duration: 6000 });
  ...
  async function handleAddPhone() {
    ...
    if (d.length < 10) { toast.error('Enter a valid 10-digit number'); return; }
    setPhoneBusy(true);
    const ok = await addPhoneToAccount(newPhone);
    setPhoneBusy(false);
    if (ok) {
      setAddingPhone(false);
      setNewPhone('');
      toast.success('Phone number saved! Now verify it via WhatsApp.');
      // Immediately trigger OTP
      await handleStartPhoneVerify(normalisePhone(newPhone));
    }
  }

  // ── Send WhatsApp OTP ────────────────────────────────────────────────────
  async function handleStartPhoneVerify(overridePhone?: string) {
    const ph = overridePhone ?? user.phone;
    if (!ph) { toast.error('Add a phone number first'); return; }
    setPhoneVerifyBusy(true);
    const ok = await sendWhatsAppOtp(ph);
    setPhoneVerifyBusy(false);
    if (ok) { setPhoneVerifying(true); setPhoneVerifyOtp(''); }
  }

  // ── Verify WhatsApp OTP ──────────────────────────────────────────────────
  async function handleConfirmPhoneVerify() {
    if (phoneVerifyOtp.trim().length !== 6) { toast.error('Enter the 6-digit OTP'); return; }
    setPhoneVerifyBusy(true);
    const ok = await verifyWhatsAppOtp(user.phone!, phoneVerifyOtp);
    setPhoneVerifyBusy(false);
    if (ok) { setPhoneVerifying(false); setPhoneVerifyOtp(''); }
  }
      ...
      <div className="grid gap-5 sm:gap-6 md:grid-cols-2">
        ...
        <div className="rounded-2xl border border-border bg-card p-5 sm:p-6 space-y-5">
            ...
            {hasEmail ? (
              <>
                ...
                {!user.email_verified && (
                  <div className="rounded-xl bg-amber-50 border border-amber-200 px-3 py-3 space-y-2">
                    <p className="text-xs text-amber-700">Check your inbox and click the verification link.</p>
                    <button onClick={() => { setEmailResendBusy(true); resendVerificationEmail().finally(() => setEmailResendBusy(false)); }}
              ...
              <div className="space-y-2">
                <div className="rounded-xl bg-red-50 border border-red-200 px-3 py-3">
                  ...
                  <p className="text-xs text-red-600 mt-0.5">Required for order confirmations & to place orders.</p>
                </div>
                {!addingEmail ? (
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      <button onClick={handleAddEmail} disabled={emailBusy}
                        ...
                        <Mail className="h-3.5 w-3.5" /> {emailBusy ? 'Saving…' : 'Save & Send Verification'}
                      </button>
                      <button onClick={() => { setAddingEmail(false); setNewEmail(''); }}
          ...
          <div className="space-y-2">
            ...
            {hasPhone ? (
              <>
                ...
                {editingPhone ? (
                  <div className="flex items-center gap-2">
                    <input value={phoneVal} onChange={e => setPhoneVal(e.target.value)} type="tel" className={inputCls} autoFocus placeholder="+919876543210" />
                    <button onClick={savePhoneEdit} className="p-2 rounded-lg bg-green-50 text-green-600 hover:bg-green-100 transition-colors"><Check className="h-4 w-4" /></button>
                  ...
                  <div className="flex items-center justify-between px-3 py-2.5 rounded-lg bg-secondary border border-input">
                    <div className="flex items-center gap-2 flex-1">
                      ...
                      {user.phone_verified
                        ? <span className="flex items-center gap-1 text-xs font-bold text-green-600"><CheckCircle2 className="h-3.5 w-3.5" /> Verified</span>
                        : <span className="flex items-center gap-1 text-xs font-semibold text-amber-600"><AlertCircle className="h-3.5 w-3.5" /> Unverified</span>
                      }
                    </div>
                    {!user.phone_verified && (
                      <button onClick={() => setEditingPhone(true)} className="text-muted-foreground hover:text-foreground ml-2"><Edit2 className="h-3.5 w-3.5" /></button>
                    )}
                  </div>
                )}
                ...
                {!user.phone_verified && phoneVerifying && (
                  <div className="rounded-xl bg-green-50 border border-green-200 px-3 py-3 space-y-3">
                    ...
                    <div className="flex gap-2">
                      <button onClick={handleConfirmPhoneVerify} disabled={phoneVerifyBusy || phoneVerifyOtp.length !== 6}
                        ...
                        <CheckCircle2 className="h-3.5 w-3.5" />{phoneVerifyBusy ? 'Verifying…' : 'Confirm OTP'}
                      </button>
                      <button onClick={() => { setPhoneVerifying(false); setPhoneVerifyOtp(''); }} className="px-3 py-2 rounded-lg bg-secondary text-xs text-muted-foreground hover:bg-accent">Cancel</button>
                    </div>
                    <button onClick={() => handleStartPhoneVerify()} disabled={phoneVerifyBusy} className="text-xs text-green-700 hover:underline">Resend OTP</button>
                  </div>
                )}
              </>
            ) : (
              /* No phone — email-signup user */
              <div className="space-y-2">
                <div className="rounded-xl bg-red-50 border border-red-200 px-3 py-3">
                  <p className="text-xs text-red-700 font-semibold">No phone number added yet</p>
                  <p className="text-xs text-red-600 mt-0.5">Required to place orders and receive shipping updates.</p>
                </div>
                {!addingPhone ? (
                  ...
                  <div className="space-y-2">
                    <div className="flex gap-2">
                      ...
                      <input type="tel" inputMode="numeric" maxLength={10} autoFocus value={newPhone}
                        onChange={e => setNewPhone(e.target.value.replace(/\D/g, ''))} placeholder="9876543210"
                ...
                {phoneVerifying && (
                  <div className="rounded-xl bg-green-50 border border-green-200 px-3 py-3 space-y-3">
                    ...
                    <input type="text" inputMode="numeric" maxLength={6} autoFocus
                      ...
                      className="w-full rounded-lg border border-green-300 bg-white px-3 py-2 text-sm text-center tracking-[0.4em] font-bold focus:outline-none focus:ring-2 focus:ring-green-400" />
                    <button onClick={handleConfirmPhoneVerify} disabled={phoneVerifyBusy || phoneVerifyOtp.length !== 6}
                      className="w-full flex items-center justify-center gap-1.5 rounded-lg bg-green-600 text-white text-xs font-bold py-2 hover:bg-green-700 disabled:opacity-50">
                      <CheckCircle2 className="h-3.5 w-3.5" />{phoneVerifyBusy ? 'Verifying…' : 'Confirm OTP'}
                    </button>
                  </div>
                )}
          ...
          <div className="space-y-3 pt-2 border-t border-border">
            <button onClick={() => { setChangingPw(v => !v); setOldPw(''); setNewPw(''); setNewPwCon(''); }}
            ...
            {changingPw && (
              <div className="space-y-3 pt-1">
                {[
                  { val: oldPw, set: setOldPw, show: showOld, setShow: setShowOld, ph: 'Current password' },
                  { val: newPw, set: setNewPw, show: showNew, setShow: setShowNew, ph: 'New password (min 8 chars)' },
