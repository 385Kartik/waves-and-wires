...
const CANCEL_REASONS = ['Ordered by mistake','Found a better price elsewhere','Delivery time is too long','Changed my mind','Duplicate order','Other'];
...
export default function OrderTrackingPage() {
  ...
  async function enrichWithRefundStatus(orders: any[]): Promise<Order[]> {
    ...
    const { data: refunds } = await supabase
      .from('refund_requests').select('order_id, status').in('order_id', orders.map(o => o.id));
  ...
  async function cancelOrder(o: Order) {
    ...
    try {
      const { data, error } = await supabase.rpc('cancel_order', { p_order_id: o.id, p_reason: reason });
  ...
  async function requestRefund(o: Order) {
    ...
    setActionLoading(true);
    try {
      const { data, error } = await supabase.rpc('request_refund', { p_order_id: o.id, p_reason: refundReason.trim() });
      if (error) throw error;
      const result = data as { success: boolean; error?: string };
  ...
  function refundBadge(o: Order) {
    ...
    return <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${map[o.refund_status]??'bg-zinc-100 text-zinc-600'}`}>{labels[o.refund_status]??o.refund_status}</span>;
  }
            ...
            <div className="rounded-2xl border border-border bg-card overflow-hidden">
              <div className="bg-secondary/50 px-4 sm:px-6 py-4 sm:py-5 border-b border-border">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex flex-col items-end gap-1.5">
                    <span className={`text-xs font-bold px-3 py-1 rounded-full capitalize ${order.status==='delivered'?'bg-green-100 text-green-700':order.status==='cancelled'?'bg-red-100 text-red-700':'bg-amber-100 text-amber-700'}`}>{order.status}</span>
                    {refundBadge(order)}
                  </div>
                </div>
              </div>
                  ...
                  <div className="space-y-3">
                    {order.order_items?.map(item=>(
                      <div key={item.id} className="flex items-center gap-3">
                        {item.product_image
                          ? <img src={item.product_image} alt={item.product_name} className="h-11 w-11 sm:h-12 sm:w-12 rounded-xl object-cover border border-border shrink-0" />
          ...
          <div className="w-full max-w-sm rounded-2xl bg-card border border-border p-5 sm:p-6 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-2">
              {CANCEL_REASONS.map(reason=>(
                <label key={reason}
                  className={`flex items-center gap-3 rounded-xl border px-3 sm:px-4 py-2.5 cursor-pointer transition-all ${cancelReason===reason?'border-red-300 bg-red-50':'border-border hover:border-red-200 hover:bg-red-50/50'}`}>
                  <div className={`h-4 w-4 rounded-full border-2 shrink-0 flex items-center justify-center ${cancelReason===reason?'border-red-500':'border-muted-foreground/40'}`}>
                    {cancelReason===reason && <div className="h-2 w-2 rounded-full bg-red-500" />}
                  </div>
                  <input type="radio" className="hidden" name="cancel_reason" onChange={()=>{setCancelReason(reason);setCancelOther('');}} />
            ...
            <div className="flex gap-3">
              <button onClick={()=>{setCancelModal(null);setCancelReason('');setCancelOther('');}} disabled={actionLoading}
